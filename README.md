...
当前为 **0.2 开发验收版**：已打通 HTTP/HTTPS 转发与固定 Mock、指定域名解密、本机 CA 生成与公共证书下载。流量监控模块已提供 HTTPS 解密启停、名单管理及请求右键“当前域名解密”，设备页保留证书生成与安装。完整产品规划见 [项目计划](docs/project-plan.md)，本阶段验收见 [HTTPS 验收清单](docs/testing/https-certificates.md)，上一阶段见 [HTTP 验收](docs/testing/desktop-foundation.md)。
...
- HTTPS 解密范围支持精确域名/IP，或 *.baidu.com（根域名及所有子域名），每行一个，不含协议和端口。域名编辑器支持搜索高亮及匹配计数。修改设置只影响新连接。设备到代理协商 HTTP/1.1，暂不承诺 HTTP/2-only 客户端、HTTP/3/QUIC 或证书固定的兼容性。
