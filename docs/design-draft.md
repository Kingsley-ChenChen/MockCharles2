...
本文记录已确认需求、架构建议与待确认项。已确认内容不重复询问；未回答的事项不视为同意。初始工作区为空，无既有技术栈约束。
...
| 新 IP | 先走真实服务并应用通用 Header，在设备列表手动绑定组 |
...
| 模块 | 首版目标 | 建议补充，尚未全部纳入 |
| --- | --- | --- |
| 流量查看 | URL、方法、状态码、耗时、请求/响应 Header 和 JSON | 搜索过滤、域名树、原始/修改后对比、复制 cURL、HAR 导入导出 |
| 接口匹配 | 根据接口条件决定是否 Mock | 已确认方法/域名/路径及 Query、Header、JSON Body 条件；独立规则按可调整顺序取第一条匹配 |
| 固定 Mock | 编辑内容、从样本或业务记录生成、保存、启用/停用 | 版本、复制、校验与预览 |
| 随机 Mock | 每次请求生成列表数量和字段数据，可切换固定模式 | 类型约束、生成预览、随机种子、唯一 ID、结果追溯 |
| Header 改写 | 请求侧与响应侧增加/修改/删除；通用规则作用于真实透传流量 | 先通用规则、后接口规则，同名字段由接口规则覆盖 |
| 数据库 | 样本和业务记录管理，编辑时选取 | 导入导出、标签、脱敏、清理、版本 |
| Mock 组 | 接口顺序、当前步骤命中、未命中透传 | 执行轨迹、重置、步骤启停；结束行为已确认 |
| 移动端 | 共用代理端口、来源 IP 设备列表、按 IP 绑定组、HTTPS 证书引导 | 二维码、连通性诊断 |
| AI 管理 | Mock 数据和规则增删改查 | 生成/预览和配置编辑；运行控制首版不开放，后续再考虑 |
| 调试辅助 | 尚未确认 | 请求重发、延迟/超时/错误码模拟、禁缓存、响应差异比较 |

Charles Rewrite 官方文档列出请求/响应的 Header、URL、Query、Body 改写，可作功能对照；上述补充项是本项目建议，不表示复制全部 Charles 功能。[Charles Rewrite](https://www.charlesproxy.com/documentation/tools/rewrite/)

### 已确认的独立开关

已确认将“保存抓包记录”“启用独立 Mock”“启用组内步骤”分别设置：

- 记录：是否将该接口流量持久保存；临时展示与长期保存可分别控制。
- Mock：独立 Mock 规则是否启用。
- 组内步骤：是否纳入顺序执行；禁用的步骤在下次开始/手动重置时跳过，不改变正在运行的步骤列表。
- 响应模式：固定或每次请求随机；源数据选择发生在编辑阶段。

三个开关互不替代：关闭保存不影响匹配或返回 Mock；关闭独立 Mock 不停用组内步骤；组步骤禁用不关闭同一接口的独立 Mock 规则。运行时只按当前设备模式和对应开关处理。抓包记录关闭时，不承诺永久保存该请求正文供复查；是否保留最小执行元数据由记录策略进一步明确。

示例：订单接口保存并 Mock，登录接口保存但走真实服务，埋点不保存且透传。

## 2.1 桌面界面组织建议

建议主界面包含六个工作区，具体布局待界面设计确认：

| 工作区 | 主要操作 | 应显示的上下文 |
| --- | --- | --- |
| 设备与连接 | 配置监听端口、查看来源 IP、给设备命名、切换真实转发/独立 Mock/顺序组、选择组 | 选中设备 IP、工作模式、组名、当前步骤、已完成/运行中 |
| 实时流量 | 筛选请求、查看原始和最终 Header/JSON | 来源 IP、Mock/真实服务、命中步骤、配置版本 |
| Mock 编辑 | 选择样本/业务记录、编辑固定内容、配置随机字段、预览 | 响应模式、数据来源、未保存修改 |
| 样本与业务数据 | 保存样本、维护记录、选中记录用于生成 | 项目、集合、记录 ID、版本 |
| Mock 组 | 排序步骤、关联响应、预览、开始/停止/重置 | 组定义与选中设备的实际运行分别展示 |
| 设置与 AI 接入 | CA/HTTPS 范围、通用 Header、MCP 配置、存储 | 本机配置与项目规则的作用范围 |

一个设备绑定的组定义可以与另一设备相同，但运行进度分别展示。已确认组步骤顺序在手动重置后采用新定义；响应内容和随机规则修改后对下一次请求生效，不要求重置当前步骤。界面分别显示本次运行的步骤顺序版本与每条请求实际使用的响应版本。

流量列表建议清楚标记“真实服务 + Header 改写”与“固定 Mock”“随机 Mock”。随机响应详情显示本次实际生成内容，而不是重新生成一次预览。这样能解释手机真正收到的数据。
...
IP 是本工具看到的网络身份，不是稳定硬件标识。两个终端若经过同一中间代理或 NAT 而呈现相同来源 IP，将共享该 IP 的运行；同一终端更换 IP 或使用不同 IPv4/IPv6 地址也可能出现为另一条设备记录。首版不擅自增加 App Header 标识或不同监听端口来改变已确认方案。已确认新 IP 先走真实服务并应用通用 Header，由用户在设备列表手动绑定组，不影响其他设备。首版不自动合并 IP 或恢复原设备身份。
...
已确认关闭窗口就退出程序并停止代理；重启后不自动继续任何组运行，必须手动开始。模块仍分开组织，避免前端页面切换影响后台。已保存配置与历史记录正常持久化；在途请求退出处理、代理监听是否随启动恢复，以及是否保留上次设备绑定待生命周期细则确认。
...
1. 确认剩余产品语义及技术栈；验证两个桌面平台、手机 HTTPS 与目标 MCP 客户端。
2. 流量展示、Header 改写、样本/业务记录编辑和固定 Mock。
3. 随机模板、字段规则、预览及每请求生成，与固定模式切换。
4. 顺序 Mock 组、未命中透传、运行轨迹和重置。
5. AI CRUD 与桌面发布完善；AI 管理接口从前期统一设计，非最后临时外挂。
6. 团队共享留后续阶段。
