...
IP 是本次网络来源标识，不是硬件永久 ID。NAT 下若两台设备呈现同一来源 IP，无法按现有规则分离；IP 改变视为新设备，默认真实转发；未绑定项目的 Header 作用域待 D20。界面显示 IP、用户备注、最近活动、模式和组，不自动合并。

建议手机代理监听与管理监听分离：前者绑定所选局域网网卡；后者仅本机回环。已确认 D07：应用启动后不自动开启手机代理监听，需用户手动开启。端口值尚未确定，设备选择恢复策略仍由 D07 确认。
