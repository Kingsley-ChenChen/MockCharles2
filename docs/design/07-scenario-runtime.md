...
已确认：每 IP 独立运行，只匹配当前步骤；完整响应成功写入连接后才推进，生成/写入失败均不推进；完成后真实转发。并发等待策略及在途控制仍待 D04/D05。
...
D05 尚未确定旧响应允许完成还是立即取消。可先设计 CompletionResult 包含 obsolete_generation；日志区分“旧运行响应已写出但不提交新运行”与正常成功步骤。
...
| Header/匹配条件 | 生效细则待 D06 | 不从响应热更新要求推断 |
