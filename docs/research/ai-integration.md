...
最新确认：技术栈采用 Wails v2 + Go + Vue 3/TypeScript + SQLite。关闭桌面窗口即停止代理和本机管理执行；MCP 适配器应连接该应用实例，不能自行启动一套独立 Mock 数据库或在应用退出后延续运行。新 IP 默认真实转发，手动绑定；通用 Header 的未绑定项目作用域见 [D20](../design/decisions.md)。
