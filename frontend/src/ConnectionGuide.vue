...
let request=0;
async function refresh(){const current=++request;loading.value=true;error.value='';try{const result=await api().ConnectionInfo(props.listenAddress);if(current===request)info.value=result}catch(e){if(current===request){info.value=undefined;error.value=String(e)}}finally{if(current===request)loading.value=false}}
...
<template>
 <Teleport to="body"><dialog ref="dialog" class="connection-guide" aria-labelledby="connection-guide-title">
  ...
  <p v-if="loading" role="status">正在读取本机网卡和端口…</p>
  <p v-else-if="error" role="alert" class="error">{{error}} <button @click="refresh">重新读取</button></p>
  <template v-else-if="info">
   <p v-if="!info.listening" class="guide-warning">代理尚未开启。下方为计划使用的地址，开启代理后再连接、下载。</p>
  ...
  <p>设置好代理并开启监听后，手机浏览器输入：</p><div class="download-address"><code>http://mc.invalid</code></div>
  <p class="muted tiny">保留 http://。这是本代理提供的下载地址，需要先设置手机代理。</p>
  <h3>3. 安装并信任证书</h3>
  ...
  <p class="muted tiny">下载成功不表示设备已信任。私钥只保存在本机，下载内容仅包含公共证书。安装后，在设备页设置需要解密的域名。</p>
 </dialog></Teleport>
</template>
<style scoped>
