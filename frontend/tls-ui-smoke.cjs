...
(async()=>{
 ...
 try {
  ...
  await page.goto('http://127.0.0.1:4174');await page.getByRole('button',{name:'流量监控',exact:true}).click();
  ...
  await dialog.getByLabel('HTTPS 解密域名').fill('baidu.com\nwww.baidu.com\nhm.baidu.com');
  await dialog.getByLabel('搜索解密域名').fill('BAIDU');
  assert.equal(await dialog.locator('mark').count(),3);await dialog.getByRole('status').filter({hasText:'3 处匹配'}).waitFor();
  await dialog.getByRole('button',{name:'下一个匹配'}).click();await dialog.getByRole('status').filter({hasText:'2/3'}).waitFor();
  await page.screenshot({path:'../.cache/https-domain-search.png',fullPage:true});
  await dialog.getByLabel('搜索解密域名').fill('');await dialog.getByLabel('HTTPS 解密域名').fill('draft.test');
  ...
  await page.locator('.compact-tree .tunnels > summary').click();
  await page.locator('.compact-tree .flow-row').first().click({button:'right'});await page.getByRole('menuitem',{name:'当前域名解密',exact:true}).click();await page.getByRole('button',{name:'关闭解密',exact:true}).waitFor();
  snap=await page.evaluate(()=>window.go.main.App.Snapshot());assert.deepEqual(snap.config.tls.hosts,['retry.test','api.example.test']);
  await page.locator('.compact-tree .flow-row').first().click();await page.getByText('此记录仅为加密隧道，没有明文正文。选择“当前域名解密”并重新连接后，查看新请求。',{exact:true}).waitFor();
