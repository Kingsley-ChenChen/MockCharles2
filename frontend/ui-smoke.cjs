...
(async()=>{
 ...
 try {
  ...
  await page.mouse.move(box.x+4,box.y+80);await page.mouse.down();await page.mouse.move(box.x-116,box.y+80,{steps:6});await page.mouse.up();
  assert.equal(Number(await divider.getAttribute('aria-valuenow')),initialWidth+120,'drag widens detail');
  await divider.focus();await page.keyboard.press('ArrowRight');assert.equal(Number(await divider.getAttribute('aria-valuenow')),initialWidth+96);
  const shifted=await divider.boundingBox();await page.mouse.move(shifted.x+4,shifted.y+80);await page.mouse.down();await page.mouse.move(1,shifted.y+80,{steps:6});await page.mouse.up();
  assert.equal(await divider.getAttribute('aria-valuenow'),await divider.getAttribute('aria-valuemax'),'left panel minimum enforced');
  await page.setViewportSize({width:1100,height:760});
  await page.waitForFunction(()=>{const e=document.querySelector('[role=separator]');return Number(e.getAttribute('aria-valuenow'))<=Number(e.getAttribute('aria-valuemax'))});
  assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
  await divider.dblclick();assert.equal(Number(await divider.getAttribute('aria-valuenow')),315);
  ...
  await page.locator('.compact-tree .host > summary').first().click();
  await page.evaluate(()=>{const original=window.go.main.App.Flows;window.treePoll=0;window.go.main.App.Flows=async()=>{window.treePoll++;return original()}});
  await page.waitForFunction(()=>window.treePoll>=2);
  assert.equal(await page.locator('.compact-tree .host').first().evaluate(e=>e.open),false,'poll must preserve collapse');
  await page.getByRole('button',{name:'全部展开',exact:true}).click();
  assert.equal(await page.locator('.compact-tree .path').first().evaluate(e=>e.open),true);
  await page.getByRole('button',{name:'全部折叠',exact:true}).click();
  assert.equal(await page.locator('.compact-tree .host').first().evaluate(e=>e.open),false);
  await page.getByRole('button',{name:'全部展开',exact:true}).click();
