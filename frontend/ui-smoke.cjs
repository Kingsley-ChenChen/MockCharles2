...
(async()=>{
 ...
 try {
  ...
  await page.addInitScript(()=>{
   ...
   window.go={main:{App:{PrepareConnection:async()=>({addresses:[{ip:'192.168.1.20',interfaceName:'Wi-Fi'},{ip:'10.8.0.2',interfaceName:'VPN'}],port:'8888',listening:true}),ConnectionInfo:async()=>({addresses:[{ip:'192.168.1.20',interfaceName:'Wi-Fi'},{ip:'10.8.0.2',interfaceName:'VPN'}],port:'8888',listening:true}),CertificateInfo:async()=>({available:certAvailable,fingerprint:'TEST-FINGERPRINT',notAfter:'2031-01-01'}),GenerateCertificate:async()=>{certAvailable=true;return {available:true,fingerprint:'TEST-FINGERPRINT',notAfter:'2031-01-01'}},ExportCertificate:async()=>'',Snapshot:async()=>JSON.parse(JSON.stringify({config,proxyAddress:address})),SaveConfig:async(c,r)=>{if(r!==config.revision)throw Error('revision conflict'); config={...JSON.parse(JSON.stringify(c)),revision:r+1};},StartProxy:async(a)=>{address=a},StopProxy:async()=>{address=''},Flows:async()=>JSON.parse(JSON.stringify(flows)),ClearFlows:async()=>{flows=[]}}}};
  });
  ...
  await page.getByRole('button',{name:'开启代理',exact:true}).waitFor();
  ...
  assert.equal((await page.evaluate(()=>window.go.main.App.Snapshot())).proxyAddress,'');
